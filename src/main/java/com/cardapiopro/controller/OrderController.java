...
public class OrderController {
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "404", description = "Pedido não encontrado", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        @GetMapping("/{id}")
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "404", description = "Produto ou addon não encontrado", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        @PostMapping
        public ResponseEntity<OrderResponse> create(@Valid @RequestBody CreateOrderRequest request) {
                ...
                List<OrderItemRequest> items = request.items().stream()
                                .map(item -> new OrderItemRequest(
                                                ...
                                                item.quantity(),
                                                item.notes(),
                                                item.addons() != null ? item.addons().stream()
                                                                .map(a -> new OrderAddonRequest(a.addonId(),
                                                                                a.quantity()))
                                                                .toList()
                                                                : null))
                                .toList();

                Order savedOrder = orderService.create(order, items, request.couponCode());
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "400", description = "Transição de status inválida", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Pedido não encontrado", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "400", description = "Pedido não pode ser cancelado", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
